...
class TelegramDictService extends TelegramBaseService {
  ...
  canHandle(update: TelegramBot.Update): boolean {
    ...
    return update.message.text?.includes('dict') ?? false;
  }
  async handle(update: TelegramBot.Update): Promise<void> {
    const dictUsers = await testCodeModel.getUserByStatus('DICTATION');
...
export const telegramDictService = new TelegramDictService();
